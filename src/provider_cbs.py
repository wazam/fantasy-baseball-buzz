...
def cbs_startup(url_tab):
    ...
    results_page = beautifulsoup_scrape(url, '', 'div', 'Page-shell', False)
    return results_page
...
def cbs_finishup(elements_tabs, column):
    for tab in elements_tabs:
        ...
        elements_positions = beautifulsoup_find(results_page, 'all', 'a', 'Dropdown-link', True)
        for position in elements_positions:
            url_tab = str(position['href'])
            url = url_base + url_tab
            results_table = beautifulsoup_scrape(url, '', 'div', 'TableBaseWrapper', False)
            ...
            start = column - 1
            elements_7day_change = beautifulsoup_find(results_table, 'all', 'td', 'TableBase-bodyTd', False)[start::column]
